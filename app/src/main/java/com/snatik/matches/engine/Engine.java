	...
	public void onEvent(FlipCardEvent event) {
		...
		if (mFlippedId == -1) {
			...
			if (mPlayingGame.boardArrangment.isPair(mFlippedId, id)) {
				...
				Shared.eventBus.notify(new HidePairCardsEvent(mFlippedId, id), 100);
				// play music
				...
				mToFlip -= 2;
				if (mToFlip == 0) {
					int passedSeconds = (int) (Clock.getInstance().getPassedTime() / 100);
					...
					Shared.eventBus.notify(new GameWonEvent(gameState), 120);
				}
			} else {
				// Log.i("my_tag", "Flip: all down");
				// send event - flip all down
				Shared.eventBus.notify(new FlipDownCardsEvent(), 100);
			}
			mFlippedId = -1;
